// crates/cli/src/shims/gem.rs
// PATH shim for `gem install` — intercepts the RubyGems ecosystem.
...
fn main() {
    ...
    let exit_code = tokio::runtime::Builder::new_current_thread()
        ...
        .block_on(async {
            ...
            if !matches!(sub, "install" | "i") {
                return run_real(&args[1..]).await;
            }
            run_verified_install("rubygems", &args).await
        });
...
async fn run_verified_install(ecosystem: &str, args: &[String]) -> i32 {
    // gem install rails devise --version 7.0 → extract gem names
...
async fn run_real(args: &[String]) -> i32 {
    ...
    let real = match which::which_all("gem").ok().and_then(|mut it| {
        it.next();
        it.next()
    }) {
        Some(p) => p,
        None => {
            eprintln!("chain-registry: real gem not found");
            return 127;
        }
    };
